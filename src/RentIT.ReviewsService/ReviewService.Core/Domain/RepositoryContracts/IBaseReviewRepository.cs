...
public interface IBaseReviewRepository
{
    ...
    Task<IReadOnlyCollection<Review>> GetReviewsByConditionAsync(Expression<Func<Review, bool>> conditionExpression, CancellationToken cancellationToken = default);
    ...
    Task<IReadOnlyCollection<Review>> GetReviewsByEquipmentIdAsync(Guid equipmentId, CancellationToken cancellationToken = default);
}
